...
def get_bid_plan(db: Session):
    bid = nav_models.Bid
    confirm_query= db.query(nav_models.BidOwnerConfirm).filter(nav_models.BidOwnerConfirm.is_confirm == True)
    confirm = aliased(confirm_query.subquery(),"confirm")
    bids = nav_repo.select_bids(db).join(confirm, bid.id == confirm.columns.bid_id)\
        .with_entities(bid.id, bid.customer_id, bid.cargo_id, bid.point_id, extract('month',\
             nav_models.Bid.end_date).label("month"), extract('year', nav_models.Bid.end_date).label("year"),\
             extract('day', nav_models.Bid.end_date).label("decade") )
...
def get_bid_plan_stat(db: Session,year: int, month: int, customer_id: int):
    ...
    print(bids.all()[0].id)
    by_year =  bids if year == 0 else bids.filter(extract('year', nav_models.Bid.end_date) == year)
    by_month = by_year if month == 0 else by_year.filter(extract('month', nav_models.Bid.end_date) == month)
    res = by_month.filter(nav_models.Bid.customer_id == customer_id) if customer_id !=0 else by_month
